...
class SpanishMod180TaxReportCustomHandler(models.AbstractModel):
    _name = 'l10n_es.mod180.tax.report.handler'
    _description = 'Tax Report (Mod 180)'
    _inherit = 'account.report.custom.handler'

    def _query_mod_180(self, options):
        ...
        sql = """drop view if exists modelo_180_report;
            create view modelo_180_report as (
                with consulta as(
                    ...
                    with impuestos_irpf as(
                        ...
                        where
                            account_account_tag_id in (select id from account_account_tag where name ilike 'mod115%')
                    ...
                    (
                        select 
                            ...
                            partner_id, 'base' as tipo, 
                            account_tax_id as tax_id, 
                            sum(balance) as balance 
                        from 
                            account_move_line aml 
                        inner join 
                            account_move_line_account_tax_rel amlatr on (aml.id = amlatr.account_move_line_id) 
                        where 
                            account_tax_id in (select account_tax_id from impuestos_irpf) """ + where_cond + """ 
                        group by 
                            account_tax_id, partner_id, anio
                        union
                        select 
                            extract(year from date) as anio, 
                            partner_id, 'cuota' as tipo, 
                            tax_line_id as tax_id, 
                            sum(balance) as balance 
                        from 
                            account_move_line aml 
                        where 
                            tax_line_id in (select account_tax_id from impuestos_irpf) """ + where_cond + """ 
                    ...
                    select
                        ...
                        sum((case when tipo = 'base' then balance else 0 end)) as rend_dinerario,
                        (select -amount from account_tax where id = d.tax_id) as porcentaje,
                        sum((case when tipo = 'cuota' then -balance else 0 end)) as cuota_dineraria
                    from 
                        datos_115 d 
                    group by 
                        anio, partner_id, clave, tax_id

                )
            select  ROW_NUMBER() over (order by consulta.anio::text, consulta.name_partner)as id, * from consulta
            );
            select * from modelo_180_report """ + order_by + """;"""

        self._cr.execute(sql)

        return self._cr.fetchall()

    def _get_mod_180_line(self, options, row):
        report = self.env['account.report']
        return {
            'id': report._get_generic_line_id('modelo.180.report', row[0]),
            'name': row[3],
            'level': 3,
            'columns': [
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals):
        ...
        for row in query_res:
            lines.append(self._get_mod_180_line(options, row))
